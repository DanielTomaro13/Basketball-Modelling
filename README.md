...
[![Screenshot of the live site](.github/screenshot.png)](https://danieltomaro13.github.io/Basketball-Modelling/)
...
The WNBA sources from ESPN (cloud-reachable), so `--league wnba` runs anywhere including CI.
SuperCoach has no WNBA competition, so `fantasy-wnba.json` is emitted empty-but-valid (the site
renders a graceful empty state); the bookmaker `odds.json` / `futures-odds.json` WNBA sections fill
in when a book offers those markets.
